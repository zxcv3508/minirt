...
/*   Updated: 2021/04/01 20:21:20 by hyopark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minirt.h"
//#include "../mlx/mlx.h"
...
double	hit_sph(t_sp o, t_ray r, t_rec *rec)
{
	t_vec oc ;
	...
	if (d > 0)
	{
		...
		if (tmp < rec->t_max && tmp > rec->t_min)
		{
			rec->t = tmp;
			rec->t_max = tmp;
		...
		//	if(vec_dot(r.dir, rec->n))
		//	{
		//		rec->n = vec_mul(rec->n, -1.0);
		//		rec->ff = 0;
		//	}
		//	else
		//	{
...
double	hit_pl(t_pl o, t_ray r, double t_min, double t_max, t_rec *rec)
{

	double d;
	
	d = (vec_dot(make_vec_dot(o.o), o.nv)) - vec_dot(make_vec_dot(r.ori), make_vec_dot(o.o)) - vec_dot(r.dir, make_vec_dot(o.o)) ;
	if(d < 0.0)
	{
		rec->n = make_vec_dot(dot_plu(rec->p, dot_mul(make_dot(-1,-1,-1), o.o)));
		return (make_rgb(o.c));
...
{
 	if (lst->type == SP)
		return (hit_sph(*((t_sp *)lst->obj),r,rec));
//	if (lst->type == PL)
	//	return (hit_pl(*((t_pl *)(lst->obj)),r,rec));
/* 	if (lst->type == TR)
...
//	t_l *light =(t_l *)(world->l->obj);
	...
	return (light->r * fmax(vec_dot(rec->n, unit_vec(r_l.dir)),0.0));
}
...
void	init_rec(t_rec *rec)
{
	rec->p = make_dot(0.0, 0.0, 0.0);
	rec->n = make_vec(0.0, 0.0, 0.0);
	rec->t = 0;
	rec->t_max = 0xfffff;
	rec->t_min = 0;
	rec->ff = 0;
