...
double	make_diffuse(t_world *world, t_ray r, t_rec *rec, t_l *light)
{
	...
	r_l.ori = rec->p;
	r_l.dir = vec_unit(vec_sub((light->o), (rec->p)));
	return (light->r * fmax(vec_dot(vec_unit(rec->normal), vec_unit(r_l.dir)),0.0));
}
...
//	t_l *light =(t_l *)(world->l->obj);
	...
	return (light->r * fmax(pow(vec_dot(vec_ref(rec->normal, r_l.dir), vec_unit(r.dir)),30), 0.0));// famx , r.dir * -1!!!
}
