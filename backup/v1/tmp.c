...
{
	if (fabs(vec.p.x) == 1 && fabs(vec.p.y) == 0 && fabs(vec.p.z) == 0)
		return (100);
	else if (fabs(vec.p.x) == 0 && fabs(vec.p.y) == 1 && fabs(vec.p.z) == 0)
		return (101);
	else if (fabs(vec.p.x) == 0 && fabs(vec.p.y) == 0 && fabs(vec.p.z) == 1)
		return (102);
	return (0);
}
...
int			is_inside(t_point p1, t_point p2, t_point p3, t_point p)
{
	...
	v1 = vec_cro(vec_sub(make_vec_dot(p2),make_vec_dot(p1)), vec_sub(make_vec_dot(p), make_vec_dot(p1)));
	v2 = vec_cro(vec_sub(make_vec_dot(p2), make_vec_dot(p1)), vec_sub(make_vec_dot(p3), make_vec_dot(p1)));
	if (vec_dot(v1, v2) < 0)
		return (0);
	v1 = vec_cro(vec_sub(make_vec_dot(p3), make_vec_dot(p1)), vec_sub(make_vec_dot(p), make_vec_dot(p1)));
	v2 = vec_cro(vec_sub(make_vec_dot(p3), make_vec_dot(p1)), vec_sub(make_vec_dot(p2), make_vec_dot(p1)));
	if (vec_dot(v1, v2) < 0)
		return (0);
	v1 = vec_cro(vec_sub(make_vec_dot(p2), make_vec_dot(p3)), vec_sub(make_vec_dot(p), make_vec_dot(p3)));
	v2 = vec_cro(vec_sub(make_vec_dot(p2), make_vec_dot(p3)), vec_sub(make_vec_dot(p1), make_vec_dot(p3)));
	if (vec_dot(v1, v2) < 0)
		return (0);
	return (1);
}
...
int		is_inside_square(t_sq *square, t_point p)
{
	...
	{
		if (!(fabs(p.y - square->o.y) <= (square->r / 2.0)
			&& fabs(p.z - square->o.z) <= (square->r / 2.0)))
			return (0);
	}
	else if (is_aligned(square->nv) == 101)
	{
		if (!(fabs(p.z - square->o.z) <= (square->r / 2.0)
			&& fabs(p.x - square->o.x) <= (square->r / 2.0)))
			return (0);
	}
...
double	hit_tr(t_tr o, t_ray r, double t_min,  t_rec *rec)
{
	...
	if (fabs(d) < 0.000001)
		return (0);
	to_hit = vec_sub(o.nv, r.dir).p;
	t = vec_dot(make_vec_dot(to_hit), o.nv) / d;

	if (t < rec->t_min || t > rec->t_max)
		return (0);
	p = vec_plu(make_vec_dot(r.ori), vec_mul(r.dir, t)).p;
	if (!is_inside(o.f, o.s, o.t, p))
		return (0);
	rec->t = t;
	rec->t_max = t;
	rec->p = p;
...
double	hit_sq(t_sq o, t_ray r, double t_min,  t_rec *rec)
{
	...
	if (root < 0 || root < rec->t_min || root > rec->t_max)
		return (0);
	//p = at(root, *r);
	p = vec_plu(make_vec_dot(r.ori), vec_mul(r.dir,root)).p;
	if (!is_inside_square(&o, p))
		return (0);
	rec->n = o.nv;
	rec->t_max = root;
	rec->t = root;
...
//	t_l *light =(t_l *)(world->l->obj);
	...
	r_l.ori = rec->p;
	r_l.dir = unit_vec(vec_sub(make_vec_dot(light->o), make_vec_dot(rec->p)));//
	return (light->r * fmax(pow(vec_dot(vec_ref(unit_vec(rec->n), r_l.dir), unit_vec(r.dir)),10), 0.0));// famx , r.dir * -1!!!
}
