...
double	hit_sph(t_sp o, t_ray r, double t_min, double t_max, t_rec *rec)
{
	t_vec oc ;
...
{
 	if (lst->type == SP)
		return (hit_sph(*((t_sp *)lst->obj),r,0,0xffffff,rec));
	if (lst->type == PL)
		return (hit_pl(*((t_pl *)(lst->obj)),r,0,0xffffff,rec));
/* 	if (lst->type == TR)
...
void		cam_set(t_info *info, t_cam *c)
{
	...
	info->u = unit_vec(vec_cro(make_vec(0.0, 1.0, 0.0), info->w));
